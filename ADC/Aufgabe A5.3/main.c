...
#define start_stop if(!PORTBbits.RB0)break
#define ADC_INV_RES (uint8_t)((uint8_t)255-(ADC_RESULT()>>2&(uint8_t)255))
...
static bool ADC_Init(ADC_config_t* config)
{
    ...
    if(config->ADC_Enable)
    {
        ...
        ADCON1bits.ADNREF = config->negative_voltage_reference; //ADC Positive voltage Referecne Configuration bits
...
{
    // *** PORT CONFIG ***
    ANSELD = ANSELEbits.ANSE0 = ANSELEbits.ANSE1 = 0x00;  //Digital = 0; Analog = 1
    ...
    ANSELBbits.ANSB0 = 0; TRISBbits.TRISB0 = 0;
...
void main(void) {
    ...
    for(;;)
    {
      while(PORTBbits.RB0)
      {
        ...
        //ADC zum Port zuweisen und 8 Bit den Wert zwischen 0...256
        for(uint8_t i = 0; i <= 0x07; shift = 1 << i++)
        {
          LATD = shift;
          if(ADC_INV_RES > 0)
            delay(ADC_INV_RES);
        ...
        for(uint8_t i = 0x06; i >= 0x01; shift = 1 << i--)
        {
          LATD = shift;
          if(ADC_INV_RES > 0)
            delay(ADC_INV_RES);
