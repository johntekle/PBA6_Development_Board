...
static uint8_t CH_SEL = 0, CHN_SEL = 0;
...
uint16_t ADC_RESULT(uint8_t _CHS, uint8_t _CHSN)
{
#if defined ADC_INTERRUPT
    return ADC_result;
#elif defined ADC_POLLING
    if(CH_SEL != _CHS || CHN_SEL != _CHSN)   //if only the channel was changed.
    {
        //update the channelS
        ADCON0bits.CHS = _CHS;   
        ADCON2bits.CHSN = _CHSN;
        __delay_ms(1); //When changing channels, a delay is required before starting the next conversation.
    }
    CHN_SEL = _CHSN;
    CH_SEL = _CHS;
    ADCON0bits.GO_nDONE = true;
...
ADC_config_t adc_config = {
    ...
    0x00,   //Channel AN0 Ausgewählt ----> can be changed in the main code 
    0x0F,   //Single ended ADC converter ----> can be changed in the main code 
...
void main(void) {
    ...
    for(;;)
    {
        ...
        /*Es wird eine Differezellmessung ausgeführt werden. Im Parameter ADC_RESULT müssen die beide Kannal
        angegeben sein.*/
        signed char adc8_res = ((uint8_t)(ADC_RESULT(1,0)>>2)&(uint8_t)255);
        LATD = adc8_res;
        uint16_t adc8_resf = ((float)((adc8_res*1.65)/127.0)*1000);
        LCD_Home();
        printf("Diff. Spannung:\n   %5imV", adc8_resf);
