...
TIMER2_config_t timer2_config = {
    ...
    0x03,   //Set prescaler "00 Cl/1, 01, Cl/4,10 Cl/16,11 Cl/64".
    208,    //(1/150Hz)/(1/(2Mhz/64)) = 208
};

//***********PWM Configuration****************
void PWM_Config(void)
{
    APFCONbits.CCP1SEL = true; //Select ouput pin for PWM (Pin B0)
    TRISB0 = true; //Disable the CCPx pin output driver by setting the associated TRIS bit.
    CCP1CONbits.CCP1M = (uint8_t)0xFF; //Configure the CCP module for PWM mode
    CCPR1L = 10; //PWM duty cycle value. (1/20)*208 = 10.4 (Alo nur 5% Duty Cycle)
    TIMER2_Init(&timer2_config); //Call the Timer 2 Config Function
    while(!PIR1bits.TMR2IF); //Wait until the Timer overflows
    TRISB0 = false; //Enable the CCPx pin output driver by clearing the associated TRIS bit.
...
static void PORTs_Initialisierung(void)
{
    ...
    //Pin B0 as Digital input
    ANSELBbits.ANSB0 = false; TRISBbits.TRISB0 = true;
