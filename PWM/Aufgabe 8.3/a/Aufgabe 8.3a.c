 ...
 * Projekt: PWM Aufgabe 8.3
 * Created on 06. Mai 2021, 15:00
 */
...
#define PWM_Duty_10Bit(x) CCPR1L = (uint8_t)((x >> 2) & 0xFF); CCP1CONbits.DC1B = (uint8_t)((uint16_t)x & 0x03)
#define PWM_Duty_8Bit(x)  CCPR1L = (uint8_t)(x & 0xFF); CCP1CONbits.DC1B = (uint8_t)0x00
...
TIMER2_config_t timer2_config = {
    ...
    0x01,   //Set prescaler "00 Cl/1, 01, Cl/4,10 Cl/16,11 Cl/64".
    255,    //Maximal resolution
};
...
void PWM_Config(void)
{
    ...
    PWM_Duty_8Bit(0); //Nicht relevant
...
void main(void) {
    ...
    for(;;)
    {
        ...
        PWM_Duty_8Bit((uint8_t)(ADC_RESULT(0) >> 2));
